...
public class PersistenceService {
    ...
    private PersistenceService() {
        ...
        for (var entry : tmpMap.entrySet()) {
            try {
                var serviceMetadata = MetadataHelper.getInstance().getClassMetadata(entry.getKey());
    ...
    public synchronized void saveUploadedFile(InputStream fileStream, String fileName) throws PersistenceException {
        try {
            ...
            var tmpFile = Utils.saveToTempDir(fileStream, fileName);
            // verifica se o arquivo (classe/jar) eh valido
            var serviceMetadata = ValidationService.getInstance().validateUploadedFile(tmpFile);
            // move o arquivo para o diretorio de upload
            var destFile = Paths.get(Utils.getInstance().getUploadDir(), fileName);
    ...
    public List<MethodDescriptor> listServiceMethods(String classQualifiedName) {
        ...
        var serviceInfo = this.serviceCache.get(classQualifiedName);
    ...
    public boolean removeServiceClass(String classQualifiedName) {
        try {
            ...
            var serviceInfo = this.serviceCache.remove(classQualifiedName);
